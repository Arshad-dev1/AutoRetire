...
def create_testing_tab(notebook, home_frame=None):
    ...
    title_label.grid(row=0, column=1, columnspan=3, pady=(10, 5), sticky="w")

    # --- Environment ---
    tk.Label(frame, text="Environment").grid(row=1, column=0, sticky="e", padx=5, pady=2)
    env_var = tk.StringVar(value="UAT/DEV")
    env_entry = tk.Entry(frame, textvariable=env_var, width=15)
    env_entry.grid(row=1, column=1, sticky="w", padx=5, pady=2)
        ...
        for i, step in enumerate(sample_steps):
            if i < len(step_rows):
                ...
                for j, val in enumerate(step):
                    ...
                    entry.grid(row=6 + i, column=j, sticky="nsew", padx=1, pady=1)
                    entry.insert(0, val)
    ...
    test_button.grid(row=2, column=1, columnspan=3, sticky="ew", padx=5, pady=(10, 5))

    # --- Test Details ---
    tk.Label(frame, text="Test Details :").grid(row=3, column=0, sticky="e", padx=5, pady=2)
    test_details_var = tk.StringVar()
    test_details_entry = tk.Entry(frame, textvariable=test_details_var, width=40)
    test_details_entry.grid(row=3, column=1, columnspan=3, sticky="w", padx=5, pady=2)

    # --- Overall Status ---
    tk.Label(frame, text="Overall Status :").grid(row=4, column=0, sticky="e", padx=5, pady=2)
    status_var = tk.StringVar(value="Pass/Fail")
    status_entry = tk.Entry(frame, textvariable=status_var, width=10)
    status_entry.grid(row=4, column=1, sticky="w", padx=5, pady=2)

    # --- Test Steps Table Header ---
    table_headers = ["Test Step No.", "Test Step Description", "Action Procedure", "Actual Results", "Status"]
    for idx, header in enumerate(table_headers):
        tk.Label(frame, text=header, font=("TkDefaultFont", 10, "bold"), borderwidth=1, relief="solid", width=20).grid(row=5, column=idx, sticky="nsew", padx=1, pady=1)
    ...
    add_row_btn.grid(row=6 + len(step_rows), column=0, columnspan=5, pady=10)

    # --- Preview Section for JIRA, Report Name, Author Email ---
    preview_frame = ttk.LabelFrame(frame, text="Submission Preview")
    preview_frame.grid(row=0, column=5, rowspan=7, padx=10, pady=5, sticky="n")
    ...
    report_label.grid(row=1, column=0, sticky="w", padx=5)
    author_label = tk.Label(preview_frame, text="Author Email: ")
    author_label.grid(row=2, column=0, sticky="w", padx=5)
